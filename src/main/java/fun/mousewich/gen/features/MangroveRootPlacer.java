...
public class MangroveRootPlacer {
	...
	public MangroveRootPlacer(IntProvider trunkOffsetY) {
	...
	protected boolean canGrowThrough(TestableWorld world, BlockPos pos) {
		return TreeFeature.canReplace(world, pos) || world.testBlockState(pos, state -> state.isIn(ModTags.Blocks.MANGROVE_ROOTS_CAN_GROW_THROUGH));
	}

	protected void placeRoots(TestableWorld world, BiConsumer<BlockPos, BlockState> replacer, Random random, BlockPos pos, MangroveTreeFeatureConfig config) {
		if (world.testBlockState(pos, state -> state.isOf(ModBase.MUD.getBlock()) || state.isOf(ModBase.MUDDY_MANGROVE_ROOTS.getBlock()))) {
			replacer.accept(pos, this.applyWaterlogging(world, pos, ModBase.MUDDY_MANGROVE_ROOTS.getBlock().getDefaultState()));
		}
		else {
			if (!this.canGrowThrough(world, pos)) return;
			replacer.accept(pos, this.applyWaterlogging(world, pos, ModBase.MANGROVE_ROOTS.getBlock().getDefaultState()));
