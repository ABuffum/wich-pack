...
public class ModBlockRotStructureProcessor extends BlockRotStructureProcessor {
	public static final Codec<ModBlockRotStructureProcessor> CODEC = RecordCodecBuilder.create(
			instance -> instance.group(
					...
					.apply(instance, ModBlockRotStructureProcessor::new)
	);
	private final Optional<RegistryEntryList<Block>> rottableBlocks;
	private final float integrity;
	public ModBlockRotStructureProcessor(TagKey<Block> rottableBlocks, float integrity) {
		this(Optional.of(Registry.BLOCK.getOrCreateEntryList(rottableBlocks)), integrity);
	}
	public ModBlockRotStructureProcessor(float integrity) {
		this(Optional.empty(), integrity);
	}
	private ModBlockRotStructureProcessor(Optional<RegistryEntryList<Block>> rottableBlocks, float integrity) {
		super(integrity);
	...
	public Structure.StructureBlockInfo process(WorldView world, BlockPos pos, BlockPos pivot, Structure.StructureBlockInfo originalBlockInfo, Structure.StructureBlockInfo currentBlockInfo, StructurePlacementData data) {
		...
		return (this.rottableBlocks.isEmpty() || originalBlockInfo.state.isIn(this.rottableBlocks.get()))
				&& random.nextFloat() > this.integrity ? null : currentBlockInfo;
	}
	@Override
	protected StructureProcessorType<?> getType() { return StructureProcessorType.BLOCK_ROT; }
}