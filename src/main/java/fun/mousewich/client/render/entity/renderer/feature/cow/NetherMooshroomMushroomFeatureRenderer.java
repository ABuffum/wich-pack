...
public class NetherMooshroomMushroomFeatureRenderer<T extends NetherMooshroomEntity> extends FeatureRenderer<T, CowEntityModel<T>> {
	public NetherMooshroomMushroomFeatureRenderer(FeatureRendererContext<T, CowEntityModel<T>> featureRendererContext) { super(featureRendererContext); }
	public void render(MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, T entity, float f, float g, float h, float j, float k, float l) {
		if (!entity.isBaby()) {
			...
			if (!entity.isInvisible() || bl) {
				...
				BlockState blockState = entity.getVariant().block.getDefaultState();
