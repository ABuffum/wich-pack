...
public class AnimationUtils {
	public static void animate(SinglePartEntityModel<?> model, ModAnimation animation, long runningTime, float f, Vec3f vec3f) {
		float g = getRunningSeconds(animation, runningTime);
		for (Map.Entry<String, List<ModTransformation>> entry : animation.boneAnimations.entrySet()) {
			Optional<ModelPart> optional = getChild(model, entry.getKey());
			List<ModTransformation> list = entry.getValue();
			optional.ifPresent(part -> list.forEach(transformation -> {
				ModKeyframe[] keyframes = transformation.keyframes;
				int i = Math.max(0, MathHelper.binarySearch(0, keyframes.length, index -> g <= keyframes[index].timestamp) - 1);
				int j = Math.min(keyframes.length - 1, i + 1);
				ModKeyframe keyframe = keyframes[i];
				ModKeyframe keyframe2 = keyframes[j];
	...
	private static float getRunningSeconds(ModAnimation animation, long runningTime) {
	...
	public static Vec3f method_41823(float f, float g, float h) { return new Vec3f(f, -g, h); }
	public static Vec3f method_41829(float f, float g, float h) { return new Vec3f(f * rad, g * rad, h * rad); }
	public static Vec3f method_41822(double d, double e, double f) { return new Vec3f((float)(d - 1.0), (float)(e - 1.0), (float)(f - 1.0)); }
}
