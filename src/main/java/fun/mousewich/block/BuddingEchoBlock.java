...
public class BuddingEchoBlock extends EchoBlock {
	...
	public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
		if (random.nextInt(GROW_CHANCE) == 0) {
			...
			else if (blockState.isOf(ModBase.SMALL_ECHO_BUD.getBlock()) && blockState.get(Properties.FACING) == direction) {
				block = ModBase.MEDIUM_ECHO_BUD.getBlock();
			}
			else if (blockState.isOf(ModBase.MEDIUM_ECHO_BUD.getBlock()) && blockState.get(Properties.FACING) == direction) {
				block = ModBase.LARGE_ECHO_BUD.getBlock();
			}
			else if (blockState.isOf(ModBase.LARGE_ECHO_BUD.getBlock()) && blockState.get(Properties.FACING) == direction) {
