...
public class AllayEntity extends PathAwareEntity implements InventoryOwner {
	...
	public void readCustomDataFromNbt(NbtCompound nbt) {
		...
		if (nbt.contains("listener", NbtElement.COMPOUND_TYPE)) {
			ModVibrationListener.createCodec(this.listenerCallback).parse(new Dynamic<>(NbtOps.INSTANCE, nbt.getCompound("listener"))).resultOrPartial(LOGGER::error).ifPresent(vibrationListener -> this.gameEventHandler.setListener(vibrationListener, this.world));
		}
