...
public abstract class MultifaceGrowthBlock extends Block {
	...
	public static byte directionsToFlag(Collection<Direction> directions) {
		byte b = 0;
		for (Direction direction : directions) b = (byte)(b | 1 << direction.ordinal());
		return b;
	}
	...
	public boolean canPlaceAt(BlockState state, WorldView world, BlockPos pos) {
		...
		for (Direction direction : DIRECTIONS) {
			...
			if (!MultifaceGrowthBlock.canGrowOn(world, direction, blockPos, world.getBlockState(blockPos))) return false;
			bl = true;
		}
		return bl;
	}
	...
	public boolean canGrowWithDirection(BlockView world, BlockState state, BlockPos pos, Direction direction) {
		if (!this.canHaveDirection(direction) || state.isOf(this) && MultifaceGrowthBlock.hasDirection(state, direction)) return false;
		BlockPos blockPos = pos.offset(direction);
	...
	public BlockState withDirection(BlockState state, BlockView world, BlockPos pos, Direction direction) {
		if (!this.canGrowWithDirection(world, state, pos, direction)) return null;
		BlockState blockState = state.isOf(this) ? state : (this.isWaterlogged() && state.getFluidState().isEqualAndStill(Fluids.WATER) ? this.getDefaultState().with(Properties.WATERLOGGED, true) : this.getDefaultState());
		return blockState.with(MultifaceGrowthBlock.getProperty(direction), true);
	}

	@Override
	public BlockState rotate(BlockState state, BlockRotation rotation) {
		if (!this.hasAllHorizontalDirections) return state;
		return this.mirror(state, rotation::rotate);
	}

	@Override
	public BlockState mirror(BlockState state, BlockMirror mirror) {
		if (mirror == BlockMirror.FRONT_BACK && !this.canMirrorX) return state;
		if (mirror == BlockMirror.LEFT_RIGHT && !this.canMirrorZ) return state;
	...
	private BlockState mirror(BlockState state, Function<Direction, Direction> mirror) {
		...
		for (Direction direction : DIRECTIONS) {
			if (!this.canHaveDirection(direction)) continue;
			blockState = blockState.with(MultifaceGrowthBlock.getProperty(mirror.apply(direction)), state.get(MultifaceGrowthBlock.getProperty(direction)));
		}
		return blockState;
	}
