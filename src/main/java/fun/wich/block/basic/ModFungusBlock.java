...
public class ModFungusBlock extends PlantBlock implements Fertilizable {
	...
	public final Supplier<ModConfiguredFeature<HugeFungusFeatureConfig, ?>> feature;
	public ModFungusBlock(Settings settings, Supplier<ModConfiguredFeature<HugeFungusFeatureConfig, ?>> feature) {
		super(settings);
	...
	public boolean isFertilizable(BlockView world, BlockPos pos, BlockState state, boolean isClient) {
		Block block = this.feature.get().getRegistryEntry().value().config().validBaseBlock.getBlock();
	...
	public void grow(ServerWorld world, Random random, BlockPos pos, BlockState state) {
		this.feature.get().getRegistryEntry().value().generate(world, world.getChunkManager().getChunkGenerator(), random, pos);
	}
}
